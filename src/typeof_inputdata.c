...
{ 
    ...
    // try reading as GMRT 8-bit filterbank
    {
        rewind(fptr);
        char header_filename[1024];
        int flen = strlen(filename);
        strcpy(header_filename,filename);
        header_filename[flen-3]='\0';
        sprintf(header_filename,"%shdr",header_filename);
        FILE* hdrfile = fopen(header_filename,"r");
        fprintf(stderr,"%s\n",header_filename);
        if (hdrfile){
            fgets(cbuffer,21,hdrfile);
            if(strcmp(cbuffer,"# DATA FILE HEADER #")==0){
                char *key;
                char *val;
                // we have GMRT format data.
                machine_id=14;
                telescope_id=7;
                fprintf(stderr,"%s\n","GMRT OK");
                tstart=0;
                fgets(cbuffer,21,hdrfile);
                while (!feof(hdrfile)){
                    if(!fgets(cbuffer,100,hdrfile))break;
                    key=trim_string(strtok(cbuffer,":"));
                    fprintf(stderr,"key='%s'\n",key);
                    val=trim_string(strtok(NULL,"\n"));
                    fprintf(stderr,"val='%s'\n",val);
                    if(strcmp(key,"Source")==0){
                        strcpy(source_name,val);
                    }
                    if(strcmp(key,"MJD")==0){
                        tstart+=atof(val);
                    }
                    if(strcmp(key,"Polarizations")==0){
                        if(strcmp(val,"Total I")==0)nifs=1;
                    }

                    if(strcmp(key,"UTC")==0){
                        int h,m;
                        double s;
                        sscanf(val,"%d:%d:%lf",&h,&m,&s);
                        tstart+=(h*3600.0 + m*60.0+s)/86400.0;
                    }

                    if(strcmp(key,"Frequency Ch.1")==0){
                        fch1=atof(val);
                    }
                    if(strcmp(key,"Channel width")==0){
                        foff=atof(val);
                    }
                    if(strcmp(key,"Num Channels")==0){
                        nchans=atoi(val);
                        gmrtzap = (unsigned char*)malloc(nchans);
                        for(i=0; i < nchans;i++){
                            gmrtzap[i]=0xFF;
                        }
                    }
                    if(strcmp(key,"Sampling Time")==0){
                        tsamp=atof(val)*1e-6;
                    }
                    if(strcmp(key,"Coordinates")==0){
                        int h,m,d,am;
                        float s,as;
                        sscanf(val,"%d:%d:%f, %d:%d:%f",&h,&m,&s,&d,&am,&as);
                        src_dej = abs(d)*10000 + am*100 + as;
                        if(d < 0)src_dej = -src_dej;
                        src_raj = h*10000 + m*100 + s;
                    }

                    if(strcmp(key,"Bad Channels")==0){
                        int iz=0;
                        char *range = strtok(val,":");
                        while(range!=NULL){
                            range = strtok(NULL,",");
                            if(range==NULL)break;
                            int start,end;
